...
export class CharStat extends plugin {
  constructor() {
    super({
      ...
      rule: [
        {
          reg: `^${rulePrefix}(${constant.charData.rarity_keywords.join(
            "|"
          )})?(${constant.charData.profession_list.join(
            "|"
          )})?练度(列表|统计|查询)([0-9]*)$`,
          fnc: "charStat",
        },
  ...
  async charStat() {
    ...
    let reg = new RegExp(
      `^${rulePrefix}(${constant.charData.rarity_keywords.join(
        "|"
      )})?(${constant.charData.profession_list.join(
        "|"
      )})?练度(列表|统计|查询)([0-9]*)$`
    ...
    if (match[4]) {
      filter_info.rarity_filter =
        constant.charData.rarity_keywords_map[match[4]];
      filter_info.rarity_name = match[4];
    }
    if (match[5]) {
      filter_info.profession_filter =
        constant.charData.profession_map[match[5]];
    ...
    let sorted_result = this.getSortedCharList(
      res_data,
      filter_info.profession_filter,
      filter_info.rarity_filter,
      page_num,
  ...
  async charAnalysis() {
    ...
    if (avatar_type && avatar_id) {
      user_info.avatar_url =
        avatar_type == "ICON"
          ? getAvatarUrl(res_data.status.avatar.id)
          : getSkinAvatarUrl(res_data.status.avatar.id);
    } else {
  ...
  getSortedCharList(
      ...
      if (char_map.hasOwnProperty(charId)) {
        ...
        if (equip_map.hasOwnProperty(defaultEquipId)) {
          ...
          if (typeIcon !== "ORIGINAL") {
            let equip_level =
              char.equip[
                char.equip.findIndex((item) => item.id == char.defaultEquipId)
              ].level;
    ...
    if (rarity_filter != null) {
      combinedArray = combinedArray.filter(
        (char) => char.rarity === rarity_filter
      );
    }

    if (profession_filter != null) {
      combinedArray = combinedArray.filter(
        (char) => char.profession === profession_filter
      );
    }
  ...
  sortCharList(a, b) {
    ...
    let specialize_level_a = a.skills.reduce(
      (sum, e) => sum + Number(e.specializeLevel || 0),
      0
    );
    let specialize_level_b = b.skills.reduce(
      (sum, e) => sum + Number(e.specializeLevel || 0),
      0
    );
  ...
  analyze(data) {
    ...
    player_score_data.general = this.analyisisCharList(
      player_char_data,
      "general"
    );
    ...
    for (let pro_name in constant.charData.profession_map) {
      ...
      let filterd_char_data = player_char_data.filter(
        (char) => char.profession === pro_eng_name
      );
      player_score_data.profession.push(
        this.analyisisCharList(filterd_char_data, pro_name)
      );
    }
    ...
    for (let index in rarity_lst) {
      let filterd_char_data = player_char_data.filter(
        (char) => char.rarity === rarity_lst[index]
      );
      player_score_data.rarity.push(
        this.analyisisCharList(filterd_char_data, `${rarity_lst[index] + 1}星`)
      );
    }
    ...
    for (let rank in constant.gameData.analysis_rank) {
      if (
        player_score_data.general.score >= constant.gameData.analysis_rank[rank]
      ) {
        player_rank = rank;
      } else {
        break;
    ...
    // 特殊tag相关
    if (player_rank == "MAX") {
      tag_list.push("方舟领域大神");
    }
    if (player_rank == "ACE") {
      tag_list.push("肝败吓疯");
    }
    if (player_rank == "SSS" || player_rank == "SS") {
      tag_list.push("黄金大队");
    }
    if (player_rank == "S" || player_rank == "A") {
      tag_list.push("阵容齐整");
    }
    if (player_rank == "B") {
      tag_list.push("有待提升");
    }
    if (player_rank == "C" || player_rank == "D") {
      tag_list.push("新鲜的小萌新");
    }

    if (player_score_data.general.count >= 320) {
      tag_list.push("图鉴大佬");
    } else if (player_score_data.general.specialize_count >= 290) {
      tag_list.push("图鉴收集者");
    }

    if (player_score_data.general.specialize_count >= 150) {
      tag_list.push("专精大师");
    } else if (player_score_data.general.specialize_count >= 100) {
      tag_list.push("专精高手");
    } else if (player_score_data.general.specialize_count >= 75) {
      tag_list.push("专精熟手");
    }

    if (player_score_data.general.top_count >= 100) {
      tag_list.push("统统拉满");
    } else if (player_score_data.general.top_count >= 50) {
      tag_list.push("满级阵容");
    } else if (player_score_data.general.top_count >= 25) {
      tag_list.push("为爱拉满");
    }

    if (player_score_data.general.top_count >= 320) {
      tag_list.push("万人迷刀客塔");
    } else if (player_score_data.general.top_count >= 240) {
      tag_list.push("好感大师");
    }

    if (player_score_data.general.evo2_count >= 240) {
      tag_list.push("打开全是精二");
    } else if (player_score_data.general.evo2_count >= 150) {
      tag_list.push("精二起步");
    } else if (player_score_data.general.evo2_count >= 70) {
      tag_list.push("常用精二");
    }
  ...
  analyisisCharList(char_data_list, analysis_name) {
    let [count, evo2_count, score, specialize_count, top_count, top_favor] = [
      0, 0, 0, 0, 0, 0,
    ];
    let top_tag_list = [];
    for (let char_data of char_data_list) {
      ...
      if (
        char_data.level == 90 &&
        char_data.potentialRank == 5 &&
        current_skill_specialize_cnt == 3
      ) {
  ...
  calculateScore(char_data) {
    ...
    for (let skill of char_data.skills) {
      score +=
        constant.gameData.analysis_rule[char_data.rarity].specialize[
    ...
    // 潜能
    score +=
      char_data.potentialRank *
      constant.gameData.analysis_rule[char_data.rarity].potential;

    // 模组
    for (let equip of char_data.equip) {
      score +=
        constant.gameData.analysis_rule[char_data.rarity].equip[
          equip.level - 1
        ];
    }
  ...
  getMaxScore(data) {
    ...
    for (let key in data.charInfoMap) {
      max_score_data.score +=
        constant.gameData.analysis_rule[data.charInfoMap[key].rarity].max_score;
