...
export function get_name_from_nickname(nickname) {
  let buffer = fs.readFileSync(
    `${_path}/plugins/arknights-plugin/resources/charProfile/nickname.yaml`,
    "utf8"
  );
  let nickname_data = YAML.parse(buffer);

  let find_key = (value, inNickname = (a, b) => a.includes(b)) => {
    return Object.keys(nickname_data).find((k) =>
      inNickname(nickname_data[k], value)
    );
  };
...
export function profession_eng_to_name(eng_name) {
  function findKey(value, compare = (a, b) => a === b) {
    return Object.keys(constant.charData.profession_map).find((k) =>
      compare(constant.charData.profession_map[k], value)
    );
  }
